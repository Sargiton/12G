...
echo -e "${BLUE}================================${NC}"
echo -e "${BLUE}    ОДНА КОМАНДА - TELEGRAM БОТ ${NC}"
echo -e "${BLUE}================================${NC}"

# Проверяем, что мы в домашней директории
cd ~

# Проверяем, есть ли папка 12G
if [ ! -d "12G" ]; then
    echo -e "${RED}❌ Папка 12G не найдена!${NC}"
    echo -e "${YELLOW}Сначала запустите: curl -s https://raw.githubusercontent.com/Sargiton/12G/master/one-command-old-bot.sh | bash${NC}"
    exit 1
fi

# Переходим в папку 12G
cd 12G
...
echo -e "  Telegram бот: ${GREEN}~/12G/123/telegram-bot.cjs${NC}"
echo -e "  Логи: ${GREEN}~/12G/123/logs/telegram-bot-*.log${NC}"
echo ""
echo -e "${BLUE}================================${NC}"
